...
def read_data(lowercase_only=False):
    # Load the text data from the book file
    book_fname = 'Assignment 4/data/goblet_book.txt'
    with open(book_fname, 'r') as file:
        book_data = file.read()

    if lowercase_only:
        book_data = book_data.lower()

    book_chars = np.array(list(book_data))  # Convert to numpy array for vectorized operations
    unique_chars = np.unique(book_chars)  # Use np.unique to get unique characters, unique_chars is a numpy array
    ind_to_char = {idx: char for idx, char in enumerate(unique_chars)}  # Map container from index to character
    char_to_ind = {char: idx for idx, char in enumerate(unique_chars)}
    int_data = np.vectorize(char_to_ind.get)(book_chars)
